...
rescale_to_pal <- function(values, palette) {
  ...
  values_min + height_factor * values_range
}
...
#' @export
plot_volcano_3d <- function(color_palette = hcl.colors(n = 16, palette = "terrain"),
                            show_contours = TRUE,
                            pal_rescale = FALSE) {
  # Create sequence for x and y coordinates in meters (10m grid)
  x_seq <- (seq_len(nrow(volcano)) - 1) * 10
  y_seq <- (seq_len(ncol(volcano)) - 1) * 10
  ...
  p |> plotly::layout(
    scene = list(
      ...
      aspectratio = list(
        x = 1,
        y = 1,
        z = 1 # Use 1:1:1 ratio since all dimensions are in meters
      ),
      xaxis = list(
        title = "X (meters)",
