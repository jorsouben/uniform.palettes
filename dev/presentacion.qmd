...
```{r}
#| echo: TRUE
unipals::diagnostic_plot(batlow_df)
```
## Matlab "jet"

```{r}
unipals::diagnostic_plot(jetpal)
```

## `R` "rainbow"

```{r}
unipals::diagnostic_plot(rainbow(256))
```

## "Batlow" e "rainbow" comparadas

```{r}
unipals::compare_diagnostics(batlow_df, rainbow, pal_names = c("batlow", "rainbow"))
...
2. Ecualizar as distancias secuanciais CIEDE2000 entre pares de cores consecutivas. Necesariamente ten que facerse cas distancias acumuladas.

```{r}
nova_paleta <- function(n) {
  map_resample(paleta, space = "rgb", n_out = n, ensure_fit = FALSE)
}

# nova_paleta(32)$deltas() |> plot()
nova_paleta(40)$bands()
```
